  ...
  for (uint32_t vertex_index = 0; vertex_index + 2 < command.mesh.vertex_count; vertex_index += 3){
    ...
    for (int32_t y = 0; y < pixl_buff->height; ++y){
      for (int32_t x = 0; x < pixl_buff->width; ++x){
        ...
        float det_v0_v1 = det2d_vec4f(edge_v0_v1, sub_vec4f(p, v0));
        float det_v1_v2 = det2d_vec4f(edge_v1_v2, sub_vec4f(p, v1));
        float det_v2_v0 = det2d_vec4f(edge_v2_v0, sub_vec4f(p, v2));
